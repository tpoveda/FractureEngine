...
#include "../../../../include/Systems/Frac3DEngine/Layer0/F3D_Time.h"
...
bool F3D_Debug::Frac3D_RestartLog()
{
	time_t now;
	char* date;
...
bool F3D_Debug::Frac3D_Log(const char* message, ...)
{
	va_list argptr;
...
bool F3D_Debug::Frac3D_ErrLog(const char* message, ...)
{
	va_list argptr;
	FILE* file = fopen(GL_LOG_FILE, "a");	// Abrimos el archivo en modo Append
	if (!file)
